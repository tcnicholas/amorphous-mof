...
class Composition:
    ...
    def elements(self) -> list:
        """Get the list of unqiue elements in the system."""
    ...
    def reduced_formula_and_factor(self):
        ...
        formula = (
            f"{s}{c / factor:.0f}" for s, c in self.species_counts.items()
        )
...
class Box:
    ...
    def write_cif(self, file_path: str):
        ...
        file_str += (
            f"_cell_length_a\t\t\t{cell_params[0]:.5f}\n"
        ...
        del formula, factor, file_str, cell_params, cell_volume, a_t, symbols
        del xc, x, atom_list
    ...
    def _sort_atom_types(self, atomTypes):
        ...
        for e in typeGroups:
            if len(typeGroups[e][0]) > 1:
                typeGroups[e][1] += (
                    f"{typeGroups[e][0][0]}" + f"*{typeGroups[e][0][-1]}"
                )
            else:
        ...
        self._elementPairs = unique_pairs(self._elements)
        self._element_pair_ix = [
            [typeGroups[x[0]][1], typeGroups[x[1]][1]]
            for x in self._elementPairs
        ]
        return typeGroups
    ...
    def _extract_topology(self, dataFile):
        ...
        while lines:
            ...
            if curr_header is None:
                ...
                curr_header is not None
                and curr_header != "Atoms\n"
